...
description: cc-sdd実装用統合ブランチの未プッシュコミット群をreview slice単位のstacked PRに再構成する際に使用。「review sliceに分割」「stacked PRを作成」「PRをスタックして」「レビュー用にPRを分けて」「review slice stack」という依頼でトリガーする。dry-run（既定）でslice計画を提示し、apply指示で実際にブランチ・PRを作成する。
---

# cc-sdd Review Slice Stack

cc-sdd の `/kiro-impl` 実行後、push 済み統合ブランチの未 push コミット群を、**現在ブランチを変更せず**に新規 review trunk を起点とした stacked draft PR に再構成する。
...
- cc-sdd 実装用統合ブランチで `/kiro-impl` を実行済み（ブランチは push 済み、最新コミットは未 push）
...
$unpushedCommits = git log --oneline "origin/${implBranch}..HEAD"
```

**方針**: 現在ブランチ（`$implBranch`）は一切変更しない。`$baseCommit` から新規 review trunk を作成し、未 push コミットを cherry-pick して stacked PR に再構成する。

**停止条件**: `$unpushedCommits` が空（未 push コミットがない）の場合は停止して報告する。
...
現在ブランチ（`$implBranch`）は以降の操作で一切変更しない。
未 push コミット一覧を確認する。

```powershell
git log --oneline "origin/${implBranch}..HEAD"
```

### 6.2 review trunk の新規作成

`$baseCommit`（`$defaultBranch` との merge-base）から新規 review trunk を作成する。
...
**conflict 発生時は停止する。trivial でない conflict を勝手に解決しない。**
