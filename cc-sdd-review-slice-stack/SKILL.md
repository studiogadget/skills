...
cc-sdd の `/kiro-impl` 実行後、統合ブランチのコミット群（ベースブランチとの差分）を、**現在ブランチを変更せず**に新規 review trunk を起点とした stacked draft PR に再構成する。コミットが push 済みでも未 push でも動作する。
...
**方針**: 現在ブランチ（`$implBranch`）は一切変更しない。`$baseCommit` から新規 review trunk を作成し、対象コミット（`$baseCommit..HEAD`）を cherry-pick して stacked PR に再構成する。
