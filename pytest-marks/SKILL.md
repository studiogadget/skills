---
name: pytest-marks
description: "pytestのマーク体系を設計・実装するスキル。「テストをマークで分類したい」「CIでDockerテストをスキップしたい」「Dockerコンテナ内で実行できないテストを除外したい」「make docker-pytestでDockerテストを実行したい」「テストの実行対象を環境ごとに切り替えたい」「新しいマークを追加したい」「skipifを使いたい」「pytestの--strict-markersで未定義マークエラーが出る」という依頼で起動。マーク設計→pyproject.toml登録→テストコード付与→CI/ローカルコマンド整合の流れで実装する。"
---
...
ローカルDocker実行       （全て実行）        make docker-pytest
```
...
テストを付与するマークは**実行環境・制約の種類**で命名する。実装内容（`unit` / `integration`）ではなく、「このテストが動く／動かない条件」を名前にする。
...
| `integration` | 外部サービス・ファイルシステム・DB など実システムとの結合を含む | モック環境のみのCIジョブ |
...
> **命名原則**: 「このテストが**何を必要とするか**」を名前にする。`test_` メソッド名と重複させない。
...
- name: Run tests
  run: |
    uv run pytest tests/ \
      ...
      --cov=src/economic_impact_analyzer \
      --cov-report=xml \
      -v
```
